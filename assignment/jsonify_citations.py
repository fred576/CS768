...
def process_dataset(dataset_path, output_json_path):
    ...
    for paper_id in os.listdir(dataset_path):
        ...
        with open(os.path.join(paper_path, output_json_path), "w", encoding="utf-8") as out_file:
            json.dump(citation_list, out_file, indent=2)
        # print(f"Saved {len(all_data)} papers with citations to {output_json_path}")

if __name__ == "__main__":
    dataset_folder = "dataset_papers/"  # 👈 Replace with your actual dataset path
